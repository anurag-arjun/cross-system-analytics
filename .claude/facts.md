...
[2026-05-12] API queries (`api/queries.py`) hit `canonical_events` with `event_type='bridge_out'/'bridge_in'` directly — they do NOT use the `bridge_links` table. So the /bridge page was never blocked on bridge_links. The materialised `bridge_links` rows unlock NEW analyses (cross-chain funnel, bridge completion rate, FastBridge GA4→bridge→swap funnel) that have no API endpoint yet.
